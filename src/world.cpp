...
void World::trainSOM (int x, int y, int z) { 
	...
	if (_trainCount < _trainDur) {
		...
		xpos = vec3((float)x, (float)y, (float)z);
		ypos = vec3((float)_bmu->x, (float)_bmu->y, (float)_bmu->z);
		
        dist = glm::distance(xpos, ypos);
...
vec3 World::bmuVec3f(vec3 dims) {
	if (_bmu) {
		return vec3((float)_bmu->x * dims.x / _sizeX, (float)_bmu->y * dims.y / _sizeY, (float)_bmu->z * dims.z / _sizeZ);
	}
	else {
