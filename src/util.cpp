...
vec3 wrapvec3(vec3 vec, vec3 lo, vec3 hi) {
	return vec3( wrapf(vec.x, lo.x, hi.x), wrapf(vec.y, lo.y, hi.y), wrapf(vec.z, lo.z, hi.z) );
}

vec3 foldvec3(vec3 vec, vec3 lo, vec3 hi) {
	return vec3( foldf(vec.x, lo.x, hi.x), foldf(vec.y, lo.y, hi.y), foldf(vec.z, lo.z, hi.z) );
}

vec2 unmapvec2(vec2 vec, float lo, float hi) {
	return vec2( unmapf(vec.x, lo, hi), unmapf(vec.y, lo, hi) );
}
